...
NTL.phys.data.exercise1 <- filter(NTL.phys.data, year4 == 1999)
...
NTL.phys.data.exercise2 <- filter(NTL.phys.data, 
                                lakename == "Tuesday Lake" & 
                                  year4 %in% c(1990:1999))

view(NTL.phys.data.Tuesday)
```
...
NTL.phys.data.Ex3 <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake" & year4 %in% c(1990:1999) & daynum %in% c(181:210) )
...
view(NTL.phys.data.Ex4$temperature_C)
mean(NTL.phys.data.Ex4$temperature_C)


summary(NTL.phys.data.Ex4)
size_sum(NTL.phys.data.Ex4)
```
...
NTL.phys.data.irradiancelong <- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, names_to = "irradiance", values_to = "irradiance_concentration")
