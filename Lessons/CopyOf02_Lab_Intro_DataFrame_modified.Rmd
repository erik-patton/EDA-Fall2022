...
knitr::opts_chunk$set(echo = TRUE) #echo = true shows that all code chunks will be run when you generate a report
```
...
cars
```
```{r you can add names here to the code chunk}
...
```{r}
#Option 1
class(car_speed) #this is a method to check if the object is a data frame

#Option 2
is.data.frame(car_speed) #this is a method to check if the object is a data frame
```

How could you make it a data frame?

```{r}
df_car_speed <- as.data.frame(car_speed) #use the 'df' or 'data frame' function
df_car_speed
class(df_car_speed)
```
...
#Option 1
df <- cbind(df_car_speed,car_dist)  #similarly rows could be added using rbind()  cbind stands for column bind... puts columns side by side. must have same number of rows to use this
class(df)
df

#Or Option 2 - transform into a data frame before binding
df_car_dist <- as.data.frame(car_dist)  #op2 usually a good idea to convert to df first because sometimes will not be binding to the df or columns
...
create_df <- data.frame("speed"=car_speed,"dist"=car_dist) #this function creates data frames. can specify the column name
create_df
```
